...
export async function layoutER(ir: ERIR, options?: LayoutOptions): Promise<ERLayoutResult> {
  ...
  const elkNodes = ir.entities.map((e) => {
    ...
    const attrsW =
      e.attributes.reduce((max, a) => {
        const w = metrics.measureText(
          `${a.type} ${a.name} ${a.keys.join(',')}`,
          fontSize * 0.85,
          fontFamily,
        ).width;
  ...
  const layoutRels = ir.relationships.map((r, i) => {
    ...
    const points = gEdge?.sections?.[0]
      ? [
          {
            x: (gEdge.sections[0].startPoint?.x ?? 0) + padding,
            y: (gEdge.sections[0].startPoint?.y ?? 0) + padding,
          },
          ...(gEdge.sections[0].bendPoints ?? []).map((p) => ({
            x: p.x + padding,
            y: p.y + padding,
          })),
          {
            x: (gEdge.sections[0].endPoint?.x ?? 0) + padding,
            y: (gEdge.sections[0].endPoint?.y ?? 0) + padding,
          },
        ]
      : eA && eB
        ? [
            { x: eA.x + eA.width, y: eA.y + eA.height / 2 },
            { x: eB.x, y: eB.y + eB.height / 2 },
          ]
        : [];
    ...
    posEdges.push({
      ...
      label: r.roleLabel,
      points,
      type: r.relType === 'NON_IDENTIFYING' ? 'dotted' : 'solid',
      arrowHead: 'none',
    });
