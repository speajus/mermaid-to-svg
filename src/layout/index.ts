...
export type AnyLayoutResult = LayoutResult | SequenceLayoutResult | ClassLayoutResult | StateLayoutResult
  | ERLayoutResult | GanttLayoutResult | PieLayoutResult | MindmapLayoutResult;
...
export async function layout(ir: DiagramIR, options?: LayoutOptions): Promise<AnyLayoutResult> {
  switch (ir.type) {
    case 'flowchart': return layoutFlowchart(ir, options);
    case 'sequence': return layoutSequence(ir, options);
    case 'class': return layoutClass(ir, options);
    case 'state': return layoutState(ir, options);
    case 'er': return layoutER(ir, options);
    case 'gantt': return layoutGantt(ir, options);
    case 'pie': return layoutPie(ir, options);
    case 'mindmap': return layoutMindmap(ir, options);
    default:
