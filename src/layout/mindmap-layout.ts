...
function measureNode(node: MindmapNode, fontSize: number, fontFamily: string, metrics: any): { width: number; height: number } {
...
function layoutTree(
  ...
  fontSize: number, fontFamily: string, metrics: any,
): MindmapNodeLayout {
  ...
  if (node.children.length > 0) {
    ...
    return {
      id: node.id, nodeId: node.nodeId, label: node.label,
      type: node.type, isRoot: node.isRoot,
      x, y: parentY, width: size.width, height: size.height,
...
function computeBounds(node: MindmapNodeLayout): { minX: number; minY: number; maxX: number; maxY: number } {
...
export async function layoutMindmap(ir: MindmapIR, options?: LayoutOptions): Promise<MindmapLayoutResult> {
  const fontSize = 14;
