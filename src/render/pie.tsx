...
export function renderPieSvg(layout: PieLayoutResult, theme: Theme, idPrefix: string = 'mermaid'): string {
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width={layout.width} height={layout.height}
      ...
      {title && (
        <text x={layout.width / 2} y={24} textAnchor="middle"
          ...
          fill={theme.primaryTextColor}>{title}</text>
      )}
      {/* Slices */}
      {slices.map((s, i) => {
        const d = describeArc(centerX, centerY, radius, s.startAngle, s.endAngle);
        return (
          <path key={i} d={d} fill={s.color} stroke={theme.background} strokeWidth={2} />
        );
      })}
      ...
      {slices.map((s, i) => {
        ...
        return s.percentage > 0.04 ? (
          <text key={`lbl-${i}`} x={lx} y={ly} textAnchor="middle" dominantBaseline="central"
            fontSize={theme.fontSize * 0.8} fontWeight="bold" fontFamily={theme.fontFamily}
            fill="#fff">{pct}%</text>
        ) : null;
      })}
      {/* Legend */}
      {slices.map((s, i) => (
        <g key={`legend-${i}`}>
          <rect x={legendX} y={centerY - (slices.length * legendItemH) / 2 + i * legendItemH}
            width={14} height={14} fill={s.color} rx={2} />
          <text x={legendX + 20}
            y={centerY - (slices.length * legendItemH) / 2 + i * legendItemH + 10}
            fontSize={theme.fontSize * 0.85} fontFamily={theme.fontFamily}
            fill={theme.primaryTextColor}>{s.label}</text>
        </g>
      ))}
    </svg>
  );
