...
function StateNode({ node, theme }: { node: StateNodeLayout; theme: Theme }) {
  ...
  if (node.type === 'end') {
    return (
      <g>
        <circle cx={cx} cy={cy} r={node.width / 2} fill="none" stroke={theme.lineColor} strokeWidth={2} />
  ...
  if (node.type === 'fork' || node.type === 'join') {
    return <rect x={node.x} y={node.y} width={node.width} height={node.height}
      fill={theme.lineColor} rx={4} ry={4} />;
  }
  if (node.type === 'choice') {
    const points = `${cx},${node.y} ${node.x + node.width},${cy} ${cx},${node.y + node.height} ${node.x},${cy}`;
    return <polygon points={points} fill={style.fill} stroke={style.stroke} strokeWidth={style.strokeWidth} />;
  }
  ...
  return (
    <g>
      <rect x={node.x} y={node.y} width={node.width} height={node.height}
        fill={style.fill} stroke={style.stroke} strokeWidth={style.strokeWidth} rx={12} ry={12} />
  ...
  const element = (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox={`0 0 ${width} ${height}`}
      ...
      <g transform={`translate(${padding}, ${padding})`}>
          ...
          return (
            <g key={edge.id}>
              ...
              {edge.label && edge.points.length >= 2 && (() => {
                ...
                return (
                  <text x={pt.x + 5} y={pt.y - 5} fontSize={theme.fontSize * 0.85}
        ...
        {/* State nodes */}
        {stateNodes.map(node => <StateNode key={node.id} node={node} theme={theme} />)}
      </g>
    </svg>
  );
