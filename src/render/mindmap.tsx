...
function MindmapNodeComponent({ node, theme, depth }: { node: MindmapNodeLayout; theme: Theme; depth: number }) {
  ...
  if (node.type === 'circle' || node.isRoot) {
    ...
    shape = <ellipse cx={cx} cy={cy} rx={node.width / 2} ry={node.height / 2}
      fill={color} stroke={theme.borderColor} strokeWidth={1.5} opacity={0.85} />;
    ...
    shape = <polygon points={pts} fill={color} stroke={theme.borderColor} strokeWidth={1.5} opacity={0.85} />;
  } else if (node.type === 'cloud') {
    shape = <ellipse cx={cx} cy={cy} rx={node.width / 2 + 4} ry={node.height / 2 + 4}
      fill={color} stroke={theme.borderColor} strokeWidth={1.5} opacity={0.85}
      strokeDasharray="4,3" />;
  } else if (node.type === 'bang') {
    shape = <rect x={node.x} y={node.y} width={node.width} height={node.height}
      fill={color} stroke={theme.borderColor} strokeWidth={2} opacity={0.85}
      rx={0} ry={0} />;
  } else {
  ...
  return (
    <g>
      ...
      {node.children.map(child => (
        <line key={`link-${child.id}`}
          x1={node.x + node.width} y1={cy}
          x2={child.x} y2={child.y + child.height / 2}
          stroke={theme.lineColor} strokeWidth={1.5} opacity={0.6} />
      ))}
      {shape}
      <text x={cx} y={cy} textAnchor="middle" dominantBaseline="central"
        fontSize={node.isRoot ? theme.fontSize * 1.1 : theme.fontSize * 0.9}
        fontWeight={node.isRoot ? 'bold' : 'normal'} fontFamily={theme.fontFamily}
        fill="#fff">{node.label}</text>
      {node.children.map(child => (
...
export function renderMindmapSvg(layout: MindmapLayoutResult, theme: Theme, idPrefix: string = 'mermaid'): string {
  const svg = (
